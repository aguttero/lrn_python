...
class Agreement(Base):
    ...
    type: Mapped[str]
    status: Mapped[str]
    workflow_id: Mapped[Optional[str]] = mapped_column(ForeignKey("workflow.workflow_id"))
    group_id: Mapped[str]
    ...
    workflow: Mapped[Optional["Workflow"]] = relationship(back_populates="agreement")
...
class Workflow(Base):
    ...
    id: Mapped[int] = mapped_column(primary_key=True)
    workflow_id: Mapped[str] = mapped_column(nullable=False, unique=True, index=True)
    workflow_name: Mapped[str] = mapped_column(index=True)

    agreement: Mapped[Optional["Agreement"]] = relationship(back_populates="workflow")
    
    def __repr__(self) -> str:
        return f"Workflow(name={self.workflow_name!r}, pk_id={self.id!r}, wrkflw_id={self.workflow_id!r})"
